$Global:GoDataDirectory = "$([Environment]::GetFolderPath('LocalApplicationData'))\GoTo\"
$Global:GoRememberFileName = "goto-remember-last.txt"
$Global:GoBookmarkFileName = "goto.txt"

function goto([string]$key, [string]$selectedPath = "", [switch]$help, [switch]$h, [switch]$add, [switch]$a, [switch]$delete, [switch]$d, [switch]$clear, [switch]$c, [switch]$show, [switch]$s, [switch]$showAll, [switch]$sa, [switch]$last, [switch]$l)
{
    #------------------------------------Help------------------------------------
    if($help -or $h)
    {
